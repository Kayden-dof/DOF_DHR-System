...
/*
 * 품질책임자도 로그인한다 (사용자 결정 2026-09-11).
 *
 * 전에는 비밀번호를 주지 않았다 - "시스템 미사용, 인쇄물에 이름만" 이었다.
 * 그런데 종이에 이름이 오르는 사람이 그 종이를 시스템에서 못 봤다. 0103 이
 * 열람 경로를 열면서 이미 같은 자리를 짚었다 (§7.1).
 *
 * 여는 것은 **보는 것**뿐이다. 전자서명은 여전히 받지 않고 (§1), 판정은
 * 종이 위에서 일어난다.
 */
await user('900100', '정품질책임', ['QP'], DEMO_PIN);
/*
 * 열람 계정. 진행 상황을 보기만 한다.
 *
...
console.log('  900100  정품질책임  품질책임자     기록과 종이를 열람한다');
console.log(`
