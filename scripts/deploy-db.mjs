...
if (c.users === 0) {
  ...
  console.log('  ──────────────────────────────────────────');
  /*
   * 이 계정은 개발 계정이다. 그래야 비밀번호 초기화가 가능하다. 다만 개발
   * 계정 표시는 끌 수 없으므로 (0052), 실무에 들어가기 전에 운영용 관리자를
   * 따로 세워야 한다. 여기서 말해 두지 않으면 IQ-19 에서 처음 알게 된다.
   */
  console.log('   * 이 계정은 개발 계정이다. 표시는 끌 수 없다.');
  console.log('     실무 착수 전에 운영용 시스템관리자를 따로 만들고,');
  console.log('     기준정보는 그 계정으로 등록할 것.');
} else {
