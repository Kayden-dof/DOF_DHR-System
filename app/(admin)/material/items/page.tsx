...
import { NewItemForm, ItemRowView, type ItemRow } from '../../settings/items/item-forms';
...
export default async function MaterialItemsPage({ searchParams }: { searchParams: Search }) {
  ...
  if (blocksScreen(user, '/material/items')) return <Denied what="이 화면" need="생산관리자 또는 시스템관리자" />;
  ...
  const { items, counts, suppliers } = await withActor(user.id, async (db) => ({
    ...
    items: await db.rows<ItemRow>(
      `select i.*, count(ml.id)::int as lot_count
        ...
        where i.type <> 'FIN'
          and ($1::text is null or i.type::text = $1)
    ...
    counts: await db.rows<{ type: string; n: number }>(
      `select type::text as type, count(*)::int as n from item where type <> 'FIN' group by type`,
    ),
  }));
  ...
  const link = (t?: string) => {
    ...
    if (q) p.set('q', q);
    const s = p.toString();
    return s ? `/material/items?${s}` : '/material/items';
  };
  ...
  return (
    <PageShell
      section="자재"
      title="품목"
      lede={<>사들이는 자재입니다. 원재료 · 시약 · 공정 자재 · 포장재. 재고 · 불출 · 단가는 전부 사용 단위 기준이며, 구매 단위는 입고 등록에서만 받아 환산합니다. 만드는 제품은 <b className="text-ink">생산 &gt; 제품</b>에서 다룹니다.</>}
      action={<NewItemForm materialOnly />}
    ...
    >
      ...
      <div className="card flex flex-wrap items-center gap-2 p-3">
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          {ITEM_TYPES.filter((x) => x.code !== 'FIN').map((x) => (
